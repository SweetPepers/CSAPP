...
static pte4_t *get_entry4(pte123_t *pgd, address_t *vaddr)
{
  ...
  while (level < 4)
  {
    ...
    if (tab[vpn].present != 1)
    {
      ...
      pte123_t *new_tab = (pte123_t *)KERNEL_malloc(PAGE_TABLE_ENTRY_NUMBER * sizeof(pte123_t));
...
void kernel_pagefault_handler(address_t vaddr)
{
  ...
  uint64_t pcb_vaddr = ((thread_info_t *)thread_info_low)->pcb_vaddr;

  uint64_t pgd_paddr = ((pcb_t *)pcb_vaddr);
  ...
  int ppn = -1;
  pte4_t *victim = NULL;
  uint64_t saddr = 0xffffffffffffffff;
