...
static void TestContextSwitching()
{
  ...
  memcpy(
      (trap_frame_t *)(p2_stack_bottom + KERNEL_STACK_SIZE - sizeof(trap_frame_t)),
      &tf, sizeof(trap_frame_t));
  memcpy(
      (trap_frame_t *)(p3_stack_bottom + KERNEL_STACK_SIZE - sizeof(trap_frame_t)),
      &tf, sizeof(trap_frame_t));

  // create user frames for p2, p3
  // create contexts for p2, p3
  // actually user frames & contexts does not matter at the starting point
  p2.context.regs.rsp = p2_stack_bottom + KERNEL_STACK_SIZE - sizeof(trap_frame_t) - sizeof(user_frame_t);
  p3.context.regs.rsp = p3_stack_bottom + KERNEL_STACK_SIZE - sizeof(trap_frame_t) - sizeof(user_frame_t);
