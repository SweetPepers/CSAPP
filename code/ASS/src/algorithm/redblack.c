...
static void swap_node_content(uint64_t a, uint64_t b, rbtree_node_interface *i_node)
{
  assert(i_node != NULL);
    ...
    rbtree_node_interface *i_node)
{
  ...
  if(i_node->compare_nodes(g_left, p) == 0){
    if(i_node->compare_nodes(p_left, n) == 0){
      ...
      swap_node_content(n, g, i_node);
    }
  }else{
    if(i_node->compare_nodes(p_left, n) == 0){
      // g < n < p
      
      i_node->set_leftchild(n, g);
      i_node->set_parent(g, n);
    ...
    uint64_t node_id)
{
  ...
  //set the insert node as red node 
  ...
  while(1){
    ...
    if(i_node->is_null_node(p) == 0){ 
      ...
      if(p_color == COLOR_BLACK){
        return; //end of floating up
      }
      else{
        //p is red && n is red
        // ==> g exist and g_color is black 
